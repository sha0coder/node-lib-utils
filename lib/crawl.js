...
Crawl.prototype = {
    ...
    crawl: function(url) {
        if (this.running) {
            ...
            if (!repe) {
                if (this.debug)
                    console.log('connecting to %s',url.get());
    ...
    parseHtml: function(orig_url, ok, html) {
        ...
        if (this.onurl_callback)
            this.onurl_callback(orig_url, html);
